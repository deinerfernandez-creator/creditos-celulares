...
async function getDeviceByImei(imei) {
  ...
  if (res1.status === 200 && res1.data) {
      ...
      console.log(`Search returned ${data.devices ? data.devices.length : 0} devices`);
      if (data.devices) {
  ...
  if (res2.status === 200 && res2.data) {
      ...
      console.log(`Fallback returned ${data.devices ? data.devices.length : 0} devices`);
      if (data.devices) {
          const device = data.devices.find(d => {
             console.log("Checking device:", d.device_name, "IMEI:", d.imei);
             return d.imei === imei || (Array.isArray(d.imei) && d.imei.includes(imei))
          });
          if (device) return device;
      }
  }
