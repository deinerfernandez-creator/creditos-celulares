...
export default function WelcomePage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-body antialiased">
      ...
      {/* Hero Section */}
      <main className="flex-1 flex flex-col items-center justify-center p-6 md:p-12 space-y-12 max-w-6xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full animate-in fade-in zoom-in-95 duration-1000 delay-200">
          <Card className="group relative border-none shadow-2xl rounded-[2.5rem] overflow-hidden bg-white hover:shadow-primary/20 transition-all">
            <CardContent className="p-10 flex flex-col items-center text-center space-y-6">
              <div className="p-6 bg-accent/10 text-accent rounded-3xl group-hover:scale-110 transition-transform duration-500">
                <Calculator className="w-12 h-12" />
              </div>
              <div className="space-y-2">
                <h3 className="text-2xl font-black text-slate-900 tracking-tight">Cotizador</h3>
                <p className="text-sm text-slate-500 font-medium">Simula tu plan de pagos y descubre cuánto pagarás por tu nuevo equipo.</p>
              </div>
              <Button asChild className="w-full h-14 rounded-2xl bg-accent hover:bg-accent/90 text-white font-black text-base shadow-xl shadow-accent/20">
                <Link href="/quotations">Simular mi Crédito <ChevronRight className="w-5 h-5 ml-2" /></Link>
              </Button>
            </CardContent>
          </Card>

          <Card className="group relative border-none shadow-2xl rounded-[2.5rem] overflow-hidden bg-primary text-white hover:shadow-primary/40 transition-all md:-translate-y-4">
            <CardContent className="p-10 flex flex-col items-center text-center space-y-6">
              <div className="p-6 bg-white/10 rounded-3xl group-hover:scale-110 transition-transform duration-500">
                <Smartphone className="w-12 h-12 text-accent" />
              </div>
              <div className="space-y-2">
                <h3 className="text-2xl font-black tracking-tight">Portal Clientes</h3>
                <p className="text-sm text-white/70 font-medium">Consulta tu saldo, historial de abonos y fechas de pago en tiempo real.</p>
              </div>
              <Button asChild className="w-full h-14 rounded-2xl bg-white text-primary hover:bg-slate-50 font-black text-base shadow-xl">
                <Link href="/portal">Entrar al Portal <ChevronRight className="w-5 h-5 ml-2" /></Link>
              </Button>
            </CardContent>
          </Card>

          <Card className="group relative border-none shadow-2xl rounded-[2.5rem] overflow-hidden bg-slate-900 text-white hover:shadow-slate-900/40 transition-all">
            <CardContent className="p-10 flex flex-col items-center text-center space-y-6">
              <div className="p-6 bg-white/5 rounded-3xl group-hover:scale-110 transition-transform duration-500 text-primary">
                <ShieldCheck className="w-12 h-12" />
              </div>
              <div className="space-y-2">
                <h3 className="text-2xl font-black tracking-tight">Administración</h3>
                <p className="text-sm text-slate-400 font-medium">Acceso exclusivo para el personal de Tecnicell (Ventas y Gestión).</p>
              </div>
              <Button asChild variant="outline" className="w-full h-14 rounded-2xl border-white/20 bg-transparent hover:bg-white/5 text-white font-black text-base">
                <Link href="/login">Acceso Panel Staff <ChevronRight className="w-5 h-5 ml-2" /></Link>
              </Button>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full pt-12">
          {[
            ...
            { icon: CheckCircle2, text: "Sin Papaleos" },
